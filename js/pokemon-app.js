...
const start_s = Math.round(start_d.getTime() / 1000);
let game_over = false;
...
let map = new InteractiveMap({
	...
	update() {
		...
		if (!game_over) {
			let d = new Date();
			gameState.game_time = Math.round(d.getTime() / 1000) - start_s;
		}
		if (gameState.game_time >= 20*60) { // 20 minute time limit
			game_over = true;
		}
	},
	...
	const app = new Vue({
		...
		<header></header>
			<div id="main-columns">
				...
				<br> Points: {{gameState.points}} </br>
				<br> Time: {{gameState.game_time}}  </br>
				<br> {{gameState.messages[gameState.messages.length-1]}} </br>
				</div>
